      ...
      <div class="infoContainer">
        <div class="currentRiverName">
          ...
          <input
            type="text"
            class="firstInput"
            v-model="props.formData.zh_name"
          />
...
let select_river = props.formData.select_river; // 选择的河流
let zh_name = props.formData.zh_name; //中文名
const en_name = ref(""); //英文名
const riverCode = ref(""); //河流代码
const station_name = ref(""); //水文站信息
const naming_reason = ref(""); //命名依据
const averageFlow = ref(""); //平均流量
// 命名依据数组
const SELECT_LIST = [
  {
    value: "0",
...
const submitRiverLogForm = () => {
  ...
  const tmp = {
    ...
    // 用于存放链式结构的名字
    full_name: CHAINED_TEXT == "" ? zh_name.replace("-", "@") : CHAINED_TEXT,
    // 英文名
